...
net.Receive("impulseHL2RPAddBOL", function(len, ply)
	...
	if not ply:IsCombine() or not ply:IsUUHigherRank() then
		return
	end
	...
	if not IsValid(targ) or not targ:IsPlayer() or targ:IsCP() or targ:IsDispatchBOL() then
		return
	end
...
hook.Add("PlayerDeath", "impulseCitadelBOLRemover", function(ply, ent, attacker)
	if ( ply:IsDispatchBOL() ) then
