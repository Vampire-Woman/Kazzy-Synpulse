function meta:SquadCanMake()
	...
	if t == TEAM_CP and class < CLASS_PTL then
		return false
	end

	if t == TEAM_OTA and class < 0 then
		return false
	end
...
function meta:SquadCanClaim()
	...
	for v,k in pairs(team.GetPlayers(t)) do
		...
		if sqd and sqd == squad then
			...
			local r = k:GetTeamClass()

			if r and r != 0 and r > highestclass then
				highestclass = r
			end
		end
	end
