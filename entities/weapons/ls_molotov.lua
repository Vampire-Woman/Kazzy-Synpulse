...
function SWEP:ProjectileFire()
	...
	timer.Simple(0.2, function()
		...
		for i = 1, 23 do
			...
			molotovfire:SetKeyValue("damagescale", "3")
			...
			-- Continuous burn check
			local timerName = "MolotovBurn_" .. molotovfire:EntIndex()

			timer.Create(timerName, 0.5, 0, function()
				if not IsValid(molotovfire) then
					timer.Remove(timerName)
					return
				end

				for _, ent in ipairs(ents.FindInSphere(molotovfire:GetPos(), 100)) do
					if ent:IsPlayer() then
						ent:Ignite(math.random(3, 6))
					end
				end
			end)
