...
function ENT:Think()
  ...
  for _, ent in pairs(ents.FindInSphere(pos + (ang:Up() * 20) - (ang:Right() * 9), 20)) do
    if ent:GetClass() == "impulse_hl2rp_cloth" and not self:GetWashing() then
      if not ent:GetClean() then
        self:SetClothType(ent:GetClothType())
        self:SetWashing(true)
        self:SetWashState(impulse.Config.WashTime)
        ent:Remove()
      end
    end
  end
...
function ENT:OnWash(ent, name, ov, nv)
  ...
  timer.Create("WashTimer" .. self:EntIndex(), 1, impulse.Config.WashTime, function()
