...
function PANEL:QuickActions()
	...
	local switchAnywhere = impulse.Teams.Data[LocalPlayer():Team()].switchAnywhere or false
	if classes then
		if switchAnywhere then
			for v,classData in pairs(classes) do
				if not classData.noMenu and LocalPlayer():GetTeamClass() != v then
					...
					if classData.xp then
						btnText1 = classData.xp.."XP"
					end
					btn:SetText("Become "..classData.name.." ("..btnText1..")")
		...
		elseif LocalPlayer():IsValid() then
			for v,classData in pairs(classes) do
				if not classData.noMenu and LocalPlayer():GetTeamClass() != v then
					local btn = self.list:Add("DButton")
					if impulse.IsHighRes() then btn:SetTall(30) btn:SetFont("Impulse-Elements17-Shadow") end
					...
					if classData.lp then
						btnText = btnText.." ("..classData.lp.."LP)"
						btn:SetText(btnText)
					end
...
function PANEL:Teams()
	...
	for v,k in pairs(impulse.Teams.Data) do -- this function adds things to the team list, so I will alter it to omit certain teams - Overvisor
		if ( k.noMenu and k.noMenu == true ) then
            continue
        end
		
		local selectedList

		if(k.inMenu == false) then
			selectedList = noList
		elseif (k.xp > LocalPlayer():GetXP()) or (k.donatorOnly and k.donatorOnly == true and LocalPlayer():IsDonator() == false) then
			selectedList = unavailibleList
		else
			selectedList = availibleList
		end

		if(selectedList != noList) then
...
function PANEL:Info()
	...
	for v,k in pairs(impulse.chatCommands) do
		...
		if k.JanitorKey == true and LocalPlayer():IsJanitor() == false then 
 			continue 
 		elseif k.adminOnly == true then
 			c = impulse.Config.InteractColour
 		end

 		if (k.adminOnly == true or k.JanitorKey == true) and LocalPlayer():IsAdmin() == false then 
 			continue 
 		elseif k.adminOnly == true or k.JanitorKey == true then
 			c = impulse.Config.InteractColour
 		end
