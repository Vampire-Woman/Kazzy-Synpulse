...
function PANEL:SetItem(netitem, wide)
	...
	local panel = self
	self.model:SetPos(15, 0)
	self.model:SetSize(128, 128)
	self.model:SetMouseInputEnabled(true)
	self.model:SetModel(item.IconModel or item.Model)
	...
	self.desc:SetPos(144, 50)
	self.desc:SetSize(500, 64)
	self.desc:SetFont("inFont")
	...
	if self.IsRestricted or self.Item.Illegal then
		self.count:SetPos(16, 64)
	else
		self.count:SetPos(16, 76)
	end

	self.count:SetText("")
	self.count:SetTextColor(impulse.Config.MainColour)
	self.count:SetFont("Impulse-Elements32-Shadow")
	self.count:SetSize(64, 64)
...
function PANEL:OnMousePressed(keycode)
	...
	if self.Item.SecondaryUse then
		local shouldUse = true

		
		if shouldUse then
			popup:AddOption(self.Item.SecondaryUseName or "Use2", function()
				if self.Item.SecondaryUseWorkBarTime then
					local invid = self.InvID
					gui.EnableScreenClicker(false)
					...
					impulse.MakeWorkbar(self.Item.SecondaryUseWorkBarTime, self.Item.SecondaryUseWorkBarName or "Using2...", function()
						net.Start("impulseInvDoUse")
						net.WriteUInt(invid, 16)
						net.WriteUInt(2, 8)
						net.SendToServer()
	...
	if ((not self.IsRestricted) or (self.Item.DropIfRestricted and self.IsRestricted)) then 

		if(self.Item.Droppable) then
...
function PANEL:Paint(w, h)
	...
	if item then
		...
		surface.SetFont("Impulse-Elements27-Shadow")
		surface.SetTextPos(144, 20)
		surface.DrawText(item.Name)

		draw.SimpleText(" ", "Impulse-Elements23", w - 10, 10, color_white, TEXT_ALIGN_RIGHT)
