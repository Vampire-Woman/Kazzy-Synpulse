...
function PANEL:SetPlayer(player)
	self.Colour = Color(128,128,128)--team.GetColor(player:Team()) -- Store colour and name micro optomization, other things can be calculated on the go.
	...
	--self.modelIcon:SetModel(player.defaultModel, player:GetSkin()) -- replace with player default model
	--self.modelIcon:SetTooltip(false)
	--self.modelIcon:SetDisabled(true)

	self.profileImage = vgui.Create("AvatarImage", self)
	self.profileImage:SetPos(10,4)
	self.profileImage:SetSize(52, 52)
...
function PANEL:Paint(w,h)
	...
	surface.SetDrawColor(self.Colour)
 	...
 	if self.Player == LocalPlayer() or self.Player:GetFriendStatus() == "friend" then
		surface.SetDrawColor(128,128,128, (50 + math.sin(RealTime() * 2) * 50) * .4)
	...
	surface.SetDrawColor(darkCol)
	 ...
	 if LocalPlayer():IsAdmin() then 
	 	icName = " ("..self.Player:Name()..")  |  "..team.GetName(self.Player:Team()) 
		 -- Team name, now admin only (though cheatable, whatever)
