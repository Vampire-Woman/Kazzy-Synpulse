...
local pmCommand = {
	...
	onRun = function(ply, arg, rawText)
		...
		if plyTarget and ply != plyTarget then
			plyTarget:SendChatClassMessage(3, message, ply)
...
local radioCommand = {
	...
	requiresAlive = true,
	onRun = function(ply, arg, rawText)
		if ply:Team() == TEAM_WORKER or ply:Team() == TEAM_HAZMAT then
			return
		end	
...
local rollCommand = {
	...
	onRun = function(ply, arg, rawText)
		...
		local rollResult = math.random(1, sides)

		for _, k in pairs(player.GetAll()) do
			if (ply:GetPos() - k:GetPos()):LengthSqr() <= (impulse.Config.TalkDistance ^ 2) then
				impulse.PlayGesture(k, "g_fistshake")
			end
		end
		...
		timer.Simple(1.5, function()
			...
			for _, k in pairs(player.GetAll()) do
				if (ply:GetPos() - k:GetPos()):LengthSqr() <= (impulse.Config.TalkDistance ^ 2) then
					k:SendChatClassMessage(11, rollResult .. "|" .. sides, ply)
				end
			end
		end)
	end
...
local overwatchCommand = {
	description = "Speak in tac as Overwatch. (Admin only)",
	adminOnly = true,
	requiresArg = true,
	onRun = function(ply, arg, rawText)
...
impulse.RegisterChatCommand("/o", overwatchCommand)
...
if CLIENT then
	local rollCol = Color(235, 235, 235)
	local talkCol = Color(255, 255, 100)
	local infoCol = Color(135, 206, 250)
	local oldmanCol = Color(135, 135, 135)
	local oocTagCol = Color(137, 60, 60)
	local yellCol = Color(255, 140, 0)
	local whisperCol = Color(65, 105, 225)
	local infoCol = Color(135, 206, 250)
	local talkCol = Color(255, 255, 100)
	local radioCol = Color( 35, 214, 248 )
	local pmCol = Color(45, 154, 6)
	local advertCol = Color(255, 174, 66)
	local acCol = Color(0, 235, 0, 255)
	local eventCol = Color(255, 76, 53)
	local fagCol  = Color(255, 246, 185)
	...
	impulse.RegisterChatClass(1, function(message, speaker)
		...
		impulse.customChatFont = "synChatFont"

		if speaker:Team() == TEAM_CP then
			chat.AddText(speaker, fagCol, " says \"<:: ", message, " ::>\"")
		else
			chat.AddText(speaker, fagCol, " says ", "\"" .. message .. "\"")
		end
	end)
	...
	impulse.RegisterChatClass(2, function(message, speaker)
		impulse.customChatFont = "synChatFontItalics"
		impulse.customChatPlayer = speaker
		chat.AddText(oocTagCol, "[LOOC] ", speaker:SteamName(), oldmanCol, ": ",  message)
	end)
	...
	impulse.RegisterChatClass(8, function(message, speaker)
		impulse.customChatFont = "synChatFontCombine"
		chat.AddText(radioCol, speaker:Name(), " radios in tac ", "\"<:: " .. message .. " ::>\"")
	...
	impulse.RegisterChatClass(9, function(message, speaker)
		impulse.customChatFont = "synChatFontItalics"
		chat.AddText(fagCol, "** ", speaker:KnownName(), " ", message, ".")
	end)

	impulse.RegisterChatClass(10, function(message, speaker)
		impulse.customChatFont = "synChatFontItalics"
		chat.AddText(fagCol, "** ", message, ".")
	end)
	...
	impulse.RegisterChatClass(11, function(message, speaker)
		...
		local roll = split[1] or "0"
		local sides = split[2] or "0"
	...
	impulse.RegisterChatClass(14, function(message, speaker)
		impulse.customChatFont = "synChatFontCombine"
		chat.AddText(eventCol, "Overwatch radios in tac ", "\"<:: " .. message .. " ::>\"")
